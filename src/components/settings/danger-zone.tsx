...
export function DangerZone() {
  ...
  async function clearHistory() {
    setBusy(true);
    try {
      await api('/api/settings/reset', { method: 'POST', json: { what: 'history', confirm: 'CLEAR' } });
      toast.success('History cleared. The next run starts from scratch.');
      setOpen(false);
      setConfirm('');
      router.refresh();
    } catch (error) {
    ...
    <Card className="border-danger/30">
      <CardHeader className="pb-3">
        ...
        <CardDescription className="mt-1">
          Clearing the history forgets every decision, run, cost record and backlog snapshot. Accounts, rules,
          categories and settings stay. Nothing in Gmail changes.
        </CardDescription>
      </CardHeader>
      <CardContent>
